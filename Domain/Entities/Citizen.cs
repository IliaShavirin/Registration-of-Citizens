...
public class Citizen
{
    public Citizen(Guid id, string name, string surname, uint age, CitizenSex sex)
    {
        if (string.IsNullOrWhiteSpace(name))
            throw new RegCitzenException("Citizen name can't be empty");
        
        if (string.IsNullOrWhiteSpace(surname))
            throw new RegCitzenException("Citizen surname can't be empty");
        
        if (id == Guid.Empty)
            throw new RegCitzenException("Citizen Id can't be default");
